...
plot(perf_miv_point, add=TRUE, col="red",print.auc=FALSE)
legend("bottomright", legend=c(paste("WOE point:", format(round(auc_ROCR_bys_point,3),nsmall=3)), 
                               paste("MIV point:",format(round(auc_ROCR_miv_point,3), nsmall=3))),
       col=c(par("fg"), "red"), lwd=2)


plot(perf_bys_poly, print.auc=FALSE)
plot(perf_miv_poly, add=TRUE, col="red",print.auc=FALSE)

legend("bottomright", legend=c(paste("WOE polygon:", format(round(auc_ROCR_bys_point,3),nsmall=3)), 
                               paste("MIV polygon:",format(round(auc_ROCR_miv_poly,3), nsmall=3))),
       col=c(par("fg"), "red"), lwd=2)


# Open a pdf file
pdf("rrocplot_point.pdf") 
# 2. Create a plot
plot(perf_bys_point,colorize=FALSE)
plot(perf_miv_point, add=TRUE, col="red",print.auc=FALSE)
legend("bottomright", legend=c(paste("WOE point:", format(round(auc_ROCR_bys_point,3),nsmall=3)), 
